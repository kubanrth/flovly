...
export function MoveTaskMenu({ taskIds, currentBoardId, availableBoards, iconOnly, touch, trigger, onDone }: {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger render={trigger ?? <Button variant={iconOnly ? "ghost" : "secondary"} size={touch ? "lg" : "sm"} iconOnly={iconOnly} className={touch ? "size-11 [&_svg]:size-[18px]" : undefined} aria-label="Przenieś" title="Przenieś zadanie do innej tablicy" />}>
      ...
      <PopoverContent align="end" className="w-[300px] p-0">
        ...
        {candidates.length === 0 ? (
          ...
          <ul className="flex max-h-[300px] flex-col overflow-y-auto p-1">
            {candidates.map((b) => (
              <li key={b.id}>
                <button type="button" data-board-id={b.id} onClick={() => submit(b.id)} className="flex h-9 w-full items-center gap-2 rounded-md px-2 text-left outline-none hover:bg-n-100">
        ...
        <p className="border-t border-border px-3 py-2 text-2xs text-fg-3">
