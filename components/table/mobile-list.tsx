...
import { memberName, type BoardTableColumn, type TreeRow } from "@/components/table/types";
import { LinkedTaskList, SubtaskChecklist, hasDetail } from "@/components/table/row-details";
...
export function MobileList({
  ...
  boardId: string;
  viewId?: string;
  groups: { g: GroupBucket; rows: TreeRow[] }[];
  grouped: boolean;
            ...
            {!collapsed.has(g.key) &&
              rows.map(({ t, depth, childCount }) => {
                ...
                return (
                      ...
                      <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
                        {canExpand && (
                          <button
                            type="button"
                            aria-expanded={open}
                            aria-label={`${open ? "Zwiń" : "Rozwiń"} szczegóły zadania ${t.title}`}
                            onClick={() => onToggleExpand(t.id)}
                            className="-my-1 -ml-1 inline-flex size-6 shrink-0 items-center justify-center rounded-sm text-fg-3 outline-none"
                          >
                  ...
                  {open && hasDetail(t) && (
                    <div data-ui="list-detail" data-task-id={t.id} className="flex flex-col gap-3 border-b border-n-100 bg-canvas py-2 pr-3" style={{ paddingLeft: 12 + depth * 16 + 24 }}>
                      {t.subtasks.length > 0 && <SubtaskChecklist subtasks={t.subtasks} canEdit={canEdit} mobile />}
                      {t.linked.length > 0 && <LinkedTaskList linked={t.linked} workspaceId={workspaceId} mobile />}
                    </div>
                  )}
                  </div>
                );
